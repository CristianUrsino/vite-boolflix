...
<template>
    <div class="hero" :style="{ backgroundImage: 'url(https://image.tmdb.org/t/p/w342' + store.currentMustTreding.backdrop_path + ')' }">
            ...
            <!-- se è una serie -->
            <div v-else>
              <figure class="logo d-inline-block"><img src="./assets/images/Netflix_Mini_Logo.png" alt="logo Netflix"></figure>
              <span class="text-secondary">SERIE</span>
              <h1 class="titleHero">{{ store.currentMustTreding.name.toUpperCase() }}</h1>
            </div>
          </div>
</template>
